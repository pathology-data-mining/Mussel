...
def test_no_singledispatch_decorator():
    ...
    with open('mussel/utils/feature_extract.py', 'r') as f:
        source = f.read()
        ...
        if isinstance(node, ast.FunctionDef):
            for decorator in node.decorator_list:
                if isinstance(decorator, ast.Name):
                    assert decorator.id != 'singledispatch', \
                        f"Found @singledispatch decorator on function {node.name}"
                elif isinstance(decorator, ast.Attribute):
                    assert decorator.attr != 'register', \
...
def test_no_singledispatch_import():
    ...
    assert 'from functools import singledispatch' not in source, \
        "singledispatch is still imported"
    assert 'import singledispatch' not in source, \
        "singledispatch is still imported"
...
def test_process_dataset_uses_isinstance():
    ...
    assert 'isinstance' in func_source, \
...
def test_common_loop_logic_extracted():
    ...
    assert '_extract_features_from_loader' in source, \
        "Common loop logic not extracted"
    
    # Check that the helper is a generator (uses yield)
    tree = ast.parse(source)
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == '_extract_features_from_loader':
