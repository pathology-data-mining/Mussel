...
class TestSegmentTissueNeuralIntegration:
    def test_segment_tissue_neural_called_for_neural_seg_model(self):
        ...
        with patch(
            "mussel.utils.segment._segment_tissue_neural", return_value=fake_mask
        ) as mock_fn:
...
class TestDownloadCheckpoint:
    def test_download_called_when_no_weights_path(self):
        ...
        with (
            patch(
                "mussel.utils.neural_seg._download_checkpoint",
                return_value="/fake/deeplabv3_seg_v4.ckpt",
            ) as mock_dl,
            patch(
                "mussel.utils.neural_seg.NeuralTissueSegmenter._build_model",
    ...
    def test_download_skipped_when_weights_path_provided(self):
        ...
        with (
            patch("mussel.utils.neural_seg._download_checkpoint") as mock_dl,
            patch(
                "mussel.utils.neural_seg.NeuralTissueSegmenter._build_model",
                return_value=MagicMock(),
            ),
            ...
            seg = NeuralTissueSegmenter(
                weights_path="/provided/path.ckpt", device="cpu"
            )
            seg._ensure_model_loaded()
