...
from mussel.utils.feature_extract import extract_patch_features, filter_features
...
REF_FEATURES_PT = Path("/gpfs/cdsi_ess/foundation/reef/features/ctranspath/9481/948176.features.pt")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  {n_tiles} tiles  patch_size={attrs.get('patch_size')}  mpp={attrs.get('mpp')}")
        ...
        filt_feats_t, filt_coords = filter_features(feats_t, all_coords, classifier, CLASSIFIER_THR)
        filt_feats  = filt_feats_t.numpy()
        # filt_coords is already np.ndarray
        print(f"  After filter: {len(filt_coords)} tiles  (removed {n_tiles - len(filt_coords)})")

    # --- Load reference ---
    with h5py.File(REF_FILTER_H5, "r") as f:
        ref_coords = f["coords"][:]
    ref_feats = torch.load(REF_FEATURES_PT, map_location="cpu", weights_only=False).numpy()
    ...
    print(f"  Grid origin: Mussel x₀={x0_mussel} y₀={y0_mussel}  |  Ref x₀={x0_ref} y₀={y0_ref}  (Δy={y0_ref - y0_mussel})")
    print(f"  Exact coord overlap: {exact_overlap} / {len(ref_coords)} reference patches")
    ...
    union = ((m_xmax - m_xmin) * (m_ymax - m_ymin) +
             (r_xmax - r_xmin) * (r_ymax - r_ymin) - inter)
    ...
    print(f"    Mussel  mean={filt_feats.mean():.5f}  std={filt_feats.std():.5f}  "
          f"min={filt_feats.min():.4f}  max={filt_feats.max():.4f}")
    print(f"    Ref     mean={ref_feats.mean():.5f}  std={ref_feats.std():.5f}  "
    ...
    if exact_overlap > 0:
        ...
        am = filt_feats[mi]; ar = ref_feats[ri]
    ...
    coord_note = "exact" if exact_overlap == len(ref_coords) else f"{exact_overlap}/{len(ref_coords)} patches align"
    status = "PASS" if (0.95 <= tile_ratio <= 1.05 and bb_iou > 0.9) else (
             "WARN" if (0.90 <= tile_ratio <= 1.10 and bb_iou > 0.8) else "FAIL")
    sym = {"PASS": "✅", "WARN": "⚠️", "FAIL": "❌"}[status]
    print()
    print(f"  → {sym} {status}  tiles={len(filt_coords)}/{len(ref_coords)} ({tile_ratio:.1%})"
    ...
    print("  by tests/regression/regression_vs_reference.py (cos=1.000000 for CTransPath).")
