        ...
        with torch.no_grad():
            if slide_model_type == ModelType.TITAN_SLIDE:
                ...
                aggregated_features = model_fun(
                    features_tensor, coords_tensor, patch_size
                ).numpy()
                ...
                aggregated_features = model_fun(features_tensor, coords_tensor).numpy()
            else:
                # Other slide encoders may only need features
                aggregated_features = model_fun(features_tensor).numpy()
