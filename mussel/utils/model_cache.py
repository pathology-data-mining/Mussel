...
def setup_model_cache(cache_dir: Optional[str] = None):
    """
    Configure HuggingFace and transformers to use a persistent cache directory.

    Args:
        cache_dir: Path to persistent cache directory. If None, respects existing
                   environment variables or uses ~/.cache/huggingface

    This should be called early in the application startup.
    """
    # If environment variables are already set, respect them and return early
    if os.environ.get("HF_HOME") and cache_dir is None:
        logger.info(f"Using existing HF_HOME: {os.environ['HF_HOME']}")
    ...
    if cache_dir is None:
        # Use standard user cache directory as fallback
        cache_dir = str(Path.home() / ".cache" / "huggingface")

    os.makedirs(cache_dir, exist_ok=True)

    # Set HuggingFace cache directories to the same location
    # This ensures all models are cached in one place
    ...
    logger.info(f"Model cache configured: {cache_dir}")
