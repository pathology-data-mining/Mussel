...
def get_paths(args):
    ...
    try:
        paths["slide_path"] = slide_inventory[
            slide_inventory["image_id"] == str(args["image_id"])
        ]["slide_file"].values[0]
    except IndexError:
        return None

    sub_reef_dir = os.path.join(
        args["reef_dir"], f"{args['mpp']}_{args['patch_size']}_{args['step_size']}_None"
    )
    assert os.path.exists(sub_reef_dir), f"sub_reef_dir {sub_reef_dir} does not exist"

    paths["patch_path"] = os.path.join(
        sub_reef_dir, "patches", f"{args['image_id']}.h5"
    )
    paths["stitch_path"] = os.path.join(
        sub_reef_dir, "stitches", f"{args['image_id']}.jpg"
    )
    ...
    paths["cache_tile_indices_path"] = os.path.join(
        sub_reef_dir, "cache_tile_indices", f"{args['image_id']}.json"
    )
    paths["annot_path"] = os.path.join(sub_reef_dir, "annot", f"{args['image_id']}.csv")
    paths["annot_class_report_path"] = os.path.join(
        sub_reef_dir, "annot_class_reports", f"{args['image_id']}.html"
    )

    paths["pt_feats_path"] = os.path.join(
        sub_reef_dir, "feats", args["model_name"], "pt", f"{args['image_id']}.pt"
    )
    paths["h5_feats_path"] = os.path.join(
        sub_reef_dir, "feats", args["model_name"], "h5", f"{args['image_id']}.h5"
    )
